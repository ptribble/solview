...
public class JCpuState extends JKdemo implements ActionListener {
	...
	chipText = SolViewResources.getString("CPUSTATE.CHIP") + " ";
	coreText = SolViewResources.getString("CPUSTATE.CORE") + " ";
	cpuText = SolViewResources.getString("CPUSTATE.CPU") + " ";

	// change the labels depending on structure
	if (proctree.isMulticore()) {
	    cpuText = SolViewResources.getString("CPUSTATE.CORE") + " ";
	}
	if (proctree.isThreaded()) {
	    cpuText = SolViewResources.getString("CPUSTATE.THREAD") + " ";
	    fontScale = (orientation == SwingConstants.HORIZONTAL) ? -2 : -4;
	}
	cpuLabel = SolViewResources.getString("CPUSTATE.ABOUT.TEXT")
		+ " " + cpuText;
	cpuChartLabel = SolViewResources.getString("CPUSTATE.CHART.TEXT")
		+ " " + cpuText;
	...
	if (proctree.isMulticore() || proctree.isThreaded()) {
	    JMenu jms = new JMenu(SolViewResources.getString("SHOW.TEXT"));
	    jms.setMnemonic(KeyEvent.VK_S);
	    showChipItem = new JCheckBoxMenuItem(
			SolViewResources.getString("CPUSTATE.SHOW.CHIP"));
		...
		showCoreItem = new JCheckBoxMenuItem(
			SolViewResources.getString("CPUSTATE.SHOW.CORE"));
		...
		showThreadItem = new JCheckBoxMenuItem(
			SolViewResources.getString("CPUSTATE.SHOW.THREAD"));
	...
	extendedCpuItem[ncpu] = new JMenuItem(
			SolViewResources.getString("CPUSTATE.EXT.TEXT"));
